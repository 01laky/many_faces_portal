...
export async function getProfile(token: string | null, faceId?: number | null): Promise<ProfileMe> {
...
export async function uploadGlobalAvatar(
	...
	const base = env.apiUrl.replace(/\/$/, '');
	const res = await profileHttp.post<{ avatarUrl: string }>(`${base}/api/profile/me/avatar`, form, {
		headers: {
			...authHeaders(token),
