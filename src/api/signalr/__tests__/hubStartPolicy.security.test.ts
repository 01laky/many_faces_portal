...
describe('hubStartPolicy (PSH1-T-C02…C04)', () => {
  ...
  it('PSH1-T-C03: chat room start skipped when not member', () => {
    expect(
      shouldConnectChatRoomHub({ token: 'jwt', isMember: false, isHostViewer: false })
    ).toBe(false);
    expect(
      shouldConnectChatRoomHub({ token: 'jwt', isMember: true, isHostViewer: false })
    ).toBe(true);
    expect(
      shouldConnectChatRoomHub({ token: 'jwt', isMember: true, isHostViewer: true })
    ).toBe(false);
  });
